...
def generate_figure_panel(unpHdl, ptdHdl, latBeg, lonBeg, latEnd, lonEnd, figureName):
    ...
    fig, latLists, lonLists = cross_section_plot(
                                 ...
                                 datProj=datProj,
                                 plotProj=plotProj,
                                 planSectPlotTuple=(right_panel, (unpHdl, ptdHdl, 40000.)),#40000.)),
                                 presLevMin=10000.,
                                 xSectTitleStr=None,
                                 xLineColorList=['black']
                                )
...
x1=np.asarray(get_wrf_slp(unpHdl)).squeeze()
x2=np.asarray(get_wrf_slp(ptdHdl)).squeeze()


y1=wrf.interplevel(field3d=wrf.getvar(unpHdl,'z'),
                   vert=wrf.getvar(unpHdl,'p'),
                   desiredlev=30000.,
                   missing=np.nan,
                   squeeze=True,
                   meta=False)
y2=wrf.interplevel(field3d=wrf.getvar(ptdHdl,'z'),
                   vert=wrf.getvar(ptdHdl,'p'),
                   desiredlev=30000.,
                   missing=np.nan,
                   squeeze=True,
