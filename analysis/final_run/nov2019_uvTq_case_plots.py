...
u,v=get_uvmet(unpHdl)
spd=(u**2.+v**2.)**0.5
slev = wrf.interplevel(field3d=spd,
                            vert=wrf.getvar(unpHdl,'p'),
                            desiredlev=40000.,
                            missing=np.nan,
                            squeeze=True,
                            meta=False)
plt.contour(unpOmegaLev,levels=np.arange(0.5,3.,0.25),colors='black')
plt.contour(slev,levels=np.arange(36.,100.1,8.),colors='green')
#plt.colorbar()
plt.show()
