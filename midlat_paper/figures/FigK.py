# Figure K: Cross-section of perturbation omega, with plan-section perturbation mean 400-700 hPa geopotential height
#           perturbation and perturbation temperature advection by the geostrophic wind, and mean wind speed for
# a) least-intense simulation 3-hr forecast
# b) least-intense simulation 6-hr forecast
# c) least-intense simulation 9-hr forecast
# d) least-intense simulation 12-hr forecast
#
...
if __name__ == "__main__":
    # define unperturbed and least-intense (negative) perturbed file subdirectories
    ...
    # FIG Ka: least-intense simulation 3-hr cross section of perturbation omega
    fcstHr = 3
    ...
    # FIG Kb: least-intense simulation 6-hr cross section of perturbation omega
    fcstHr = 6
    dtFcst = dtInit + datetime.timedelta(hours=fcstHr)
    dtFcstStr = datetime.datetime.strftime(dtFcst,'%Y-%m-%d_%H:00:00')
    unpFileFcst = unpDir + 'wrfout_d01_' + dtFcstStr
    ...
    # FIG Kc: least-intense simulation 9-hr cross section of perturbation omega
    fcstHr = 9
    dtFcst = dtInit + datetime.timedelta(hours=fcstHr)
    dtFcstStr = datetime.datetime.strftime(dtFcst,'%Y-%m-%d_%H:00:00')
    unpFileFcst = unpDir + 'wrfout_d01_' + dtFcstStr
    ...
    # FIG Kd: least-intense simulation 12-hr cross section of perturbation omega
    fcstHr = 12
    dtFcst = dtInit + datetime.timedelta(hours=fcstHr)
    dtFcstStr = datetime.datetime.strftime(dtFcst,'%Y-%m-%d_%H:00:00')
    unpFileFcst = unpDir + 'wrfout_d01_' + dtFcstStr
