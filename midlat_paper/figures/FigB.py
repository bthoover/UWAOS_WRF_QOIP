...
# (b) As per panel-a, but for WRF 24-hr unperturbed forecast
...
if __name__ == "__main__":
    ...
    # FIG Ba: 0-hr unperturbed WRF forecast 2.0 PVU pot. temp, 335K wind speed, and SLP
    fcstHr = 0
    ...
    generate_figure_panel(wrfHdl, 'FIGB_panel_A')

    # FIG Ad: 24-hr unperturbed WRF forecast 2.0 PVU pot. temp, 335K wind speed, and SLP
    fcstHr = 24
    # define forecast datetime stamp
    dtFcst = dtInit + datetime.timedelta(hours=fcstHr)
    dtFcstStr = datetime.datetime.strftime(dtFcst,'%Y-%m-%d_%H:00:00')
    ...
    fileFcst = unpDir + 'wrfout_d01_' + dtFcstStr
    wrfHdl = Dataset(fileFcst)
    generate_figure_panel(wrfHdl, 'FIGB_panel_B')
#
# end
#
